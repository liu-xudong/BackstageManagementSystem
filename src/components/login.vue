<template>
  <div class="login_container">
    <div class="login_box">
      ...
      <!-- 表单区域 -->
      <el-form label-width="0px" class="login_form" :model="form" :rules="rules" ref='login_from_ref'>
        <!-- 用户名 -->
      ...
      methods:{
        ...
        login(){
          this.$refs.login_from_ref.validate(valid => {
            if(!valid) return;
            // this.$http.post('https://www.baidu.com').then(res=>{
            //   console.log(res)
            // }).catch(e=>{
            //   console.log(e)
            // })
            console.log('发送请求')
            if(this.form.username === 'liuxudong' && this.form.password === '123456'){
